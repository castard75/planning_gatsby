  ...
  const setDeleteArrayFunction = useCallback(
    (rows) => {
      const filtered = rows.filter(
        (row) =>
          searchByField(animations, row.data.id, "id_client", "id") === ""
      );
    ...
    () => [
      ...
      },
      {
        ...
        valueSetter: (params) => {
          if (!params.data) return false;
          dispatch(
            updateClients(params.data.id, { interlocuteur: params.newValue })
          );
          return params.newValue;
        },
        filter: "agTextColumnFilter",
      },
      {
        ...
        valueSetter: (params) => {
          if (!params.data) return false;
          dispatch(
            updateClients(params.data.id, { telephone: params.newValue })
          );
          return params.newValue;
        },
        filter: "agTextColumnFilter",
