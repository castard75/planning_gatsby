...
import { addAnimateurs, deleteAnimateurs, updateAnimateurs } from '../context/slicers/animateurs';
...
const Etoile = props => {
	...
	for (let i = 0; i < props.value; i++) {
		etoiles.push(<img style={{ margin: '0 2px' }} src={etoileIcon} alt='etoile' key={i} />);
	}
	...
	const setDeleteArrayFunction = useCallback(
		rows => {
			const filtered = rows.filter(
				row => searchByField(animations, row.data.id, 'id_animateur', 'id') === ''
			);
		...
		() => [
			{
				...
				checkboxSelection: params =>
					searchByField(animations, params.data.id, 'id_animateur', 'id') === '' ? true : false,
			},
			{
				headerName: 'Genre',
				field: 'genre',
				valueGetter: params => (!params.data ? '' : params.data.genre === 1 ? 'Homme' : 'Femme'),
				valueSetter: params => {
					if (!params.data) return false;
					dispatch(
						updateAnimateurs(params.data.id, { genre: params.newValue === 'Homme' ? 1 : 0 })
					);
					return params.newValue;
				},
				filter: 'agTextColumnFilter',
			...
			},
			{
				...
				filter: 'agTextColumnFilter',
				cellRenderer: memo(params => (!params.value ? '' : params.value + ' ans')),
			...
			},
			{
				...
				valueSetter: params => {
					if (!params.data) return false;
					dispatch(updateAnimateurs(params.data.id, { secteur: params.newValue }));
					return params.newValue;
				},
				filter: 'agTextColumnFilter',
				cellEditor: 'agSelectCellEditor',
			...
			},
			{
				...
				valueSetter: params => {
					if (!params.data) return false;
					dispatch(updateAnimateurs(params.data.id, { mobilite: params.newValue }));
					return params.newValue;
				},
				filter: 'agTextColumnFilter',
			},
			{
				...
				valueSetter: params => {
					if (!params.data) return false;
					dispatch(updateAnimateurs(params.data.id, { appreciation: params.newValue }));
					return params.newValue;
				},
				filter: 'agTextColumnFilter',
			...
			},
			{
				...
				valueSetter: params => {
					if (!params.data) return false;
					dispatch(updateAnimateurs(params.data.id, { commentaire: params.newValue }));
					return params.newValue;
				},
	...
	return (
		<PageLayout seo={seoParams} ref={animationParent}>
						...
						onClick={() => {
							if (window.confirm('Voulez-vous vraiment supprimer la sélection ?')) {
								...
								deleteArray.forEach(row => {
									i--;
									dispatch(deleteAnimateurs(row.data.id, i === 0 ? true : false));
								});
			...
			<Grid
				...
				rowData={animateurs !== undefined ? animateurs.filter(animateur => animateur.id !== 0) : []}
