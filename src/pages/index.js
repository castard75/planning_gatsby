...
import { addAnimations, deleteAnimations, updateAnimations } from '../context/slicers/animations';
...
import { selectAnimateurs, selectAnimations, selectClients, selectLieux } from '../context/selectors';
...
const PlanningPage = () => {
	...
	const [animations, setAnimations] = useState(filterDate(animationsInit, selectDate));
	...
	const animateursNames = useMemo(
		() => (animateurs !== undefined ? animateurs.map(animateur => animateur.nom) : []),
		[animateurs]
	);
	...
	const lieuxNames = useMemo(() => (lieux !== undefined ? lieux.map(lieu => lieu.nom) : []), [lieux]);
	...
	const searchByField = useCallback((elements, searchedValue, fieldSearch, fieldToReturn) => {
		const filter =
	...
	const setDeleteArrayFunction = useCallback(rows => setDeleteArray(rows), []);
	...
	const columnTab = useMemo(
		() => [
			{
				...
				valueSetter: params => {
					...
					const newValue = searchByField(animateurs, params.newValue, 'nom', 'id');
					dispatch(updateAnimations(params.data.id, { id_animateur: newValue }));
					return newValue;
				},
			...
			},
			{
				...
				field: 'id_client',
				valueGetter: params =>
					!params.data ? '' : searchByField(clients, params.data.id_client, 'id', 'societe'),
				valueSetter: params => {
					if (!params.data) return false;
					const newValue = searchByField(clients, params.newValue, 'societe', 'id');
			...
			},
			{
				...
				valueSetter: params => {
					if (!params.data) return false;
					dispatch(updateAnimations(params.data.id, { produit: params.newValue }));
			...
			},
			{
				...
				filterParams: {
					comparator: (filterLocalDateAtMidnight, cellValue) => {
						...
						const dateAsString = new Date(cellValue).toLocaleDateString().split('/');
			...
			},
			{
				...
				field: 'lieu',
				valueGetter: params =>
					!params.data ? '' : searchByField(lieux, params.data.id_lieu, 'id', 'nom'),
				valueSetter: params => {
					if (!params.data) return false;
			...
			},
			{
				...
				field: 'zone',
				valueGetter: params =>
					!params.data ? '' : searchByField(lieux, params.data.id_lieu, 'id', 'zone'),
				filter: 'agTextColumnFilter',
				editable: false,
			},
			{
				...
				valueSetter: params => {
					if (!params.data) return false;
					dispatch(updateAnimations(params.data.id, { horaires: params.newValue }));
					return params.newValue;
				},
				filter: 'agTextColumnFilter',
				cellRenderer: memo(props =>
			...
			},
			{
				...
				valueSetter: params => {
					if (!params.data) return false;
					dispatch(updateAnimations(params.data.id, { brief: params.newValue }));
					return params.newValue;
				},
				filter: 'agTextColumnFilter',
				cellRenderer: memo(SimpleComp),
			...
			},
			{
				...
				valueSetter: params => {
					if (!params.data) return false;
					dispatch(updateAnimations(params.data.id, { contrat: params.newValue }));
					return params.newValue;
				},
				filter: 'agTextColumnFilter',
			...
			},
			{
				headerName: 'DUE',
				field: 'due',
				valueGetter: params => (!params.data ? '' : params.data.due === 1 ? 'Oui' : 'Non'),
				valueSetter: params => {
					if (!params.data) return false;
				...
				cellRenderer: memo(params => <YesNoComp value={params.value === 'Oui' ? 1 : 0} />),
				filter: 'agTextColumnFilter',
				cellEditor: 'agSelectCellEditor',
				cellEditorParams: {
					values: ['Oui', 'Non'],
			...
			},
			{
				...
				cellEditor: 'agSelectCellEditor',
				cellEditorParams: {
					values: ['Facturé', 'Terminé', 'Validé', 'En attente', 'Annulé', 'Malade', 'Absent'],
				},
				editable: params => (params.data.export ? false : true),
				headerClass: 'dontShowWhenPrint',
				cellClass: 'dontShowWhenPrint',
		...
		[animateurs, clients, lieux, animateursNames, clientsNames, lieuxNames, dispatch, searchByField]
	);
	...
	const DateInfos = useMemo(
		() => selectDate.month.charAt(0).toUpperCase() + selectDate.month.slice(1) + ' ' + selectDate.year,
		[selectDate.month, selectDate.year]
	);
	...
	return (
		<PageLayout seo={seoParams} ref={animationParent}>
			...
			/>
			<Popup isOpen={IsOpenDateFilter} closeIsOpen={() => setIsOpenDateFilter(false)}>
				<MonthPicker
					months={lang}
					onChange={date => {
					...
					<>
						<div
							...
							<h2 style={{ marginBottom: 0 }}>
								Ajouter {duplicateAnim} {duplicateAnim !== 1 ? 'animations' : 'animation'}
							</h2>
							<DuplicateComp
								increment={() => setDuplicateAnim(lastCount => lastCount + 1)}
								decrement={() => {
									if (duplicateAnim - 1 > 0) setDuplicateAnim(lastCount => lastCount - 1);
								}}
							/>
						</div>
						<Select
							...
							onChange={e => setAddForm({ ...addForm, id_animateur: e.target.value })}
						/>
						<Select
							required
							label='Client'
							...
							onChange={e => setAddForm({ ...addForm, id_client: e.target.value })}
						/>
						<Input
							label='Produit'
							type='text'
							...
							directHandle
							reset={resetForm}
							onChange={e => setAddForm({ ...addForm, produit: e.target.value })}
						/>
						<Input
							label='Horaires'
							type='text'
							...
							directHandle
							reset={resetForm}
							onChange={e => setAddForm({ ...addForm, horaires: e.target.value })}
						/>
						<Button
							type='add'
			...
			<header>
				<div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
					<DatePickerComp className='dontShowWhenPrint' onClick={() => setIsOpenDateFilter(true)}>
						{DateInfos}
					</DatePickerComp>
			...
			<Grid
				...
				onRowClicked={params =>
					params.data && setDataAnimation({ tab: animationsInit, id: params.data.id })
				}
				rowData={animations}
