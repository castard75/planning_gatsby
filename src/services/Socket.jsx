...
const Socket = (props) => {
  ...
  const updateSocketStat = (newStat) =>
    setsocketStat((socketStat) => ({ ...socketStat, ...newStat }));
  ...
  const fetchAnimateursAction = useCallback(
    () => dispatch(fetchAnimateurs()),
    [dispatch]
  );
  const fetchClientsAction = useCallback(
    () => dispatch(fetchClients()),
    [dispatch]
  );
  const fetchLieuxAction = useCallback(
    () => dispatch(fetchLieux()),
    [dispatch]
  );
      ...
      try {
        ...
        if (!!token) {
          ...
          socketProvider.current.on("connect", () => {
            ...
            updateSocketStat({
              messageServeur:
                "Connecté au serveur, en attente d'authentification.",
            });
            ...
            //   updateSocketStat({
            //     messageServeur:
            //       "Temps d'attente dépassé, déconnexion du serveur.",
            //   });
        ...
        console.log(err);
        updateSocketStat({
          messageServeur: "Erreur lors de la connexion au serveur.",
        });
        disconnectSocket();
      ...
      if (props.pathname === "/login" || props.pathname === "/login/") {
        ...
        if (networkState.online && networkState.rtt <= 1300) {
          ...
          if (networkState.online && networkState.rtt >= 1300) {
            ...
            updateSocketStat({
              messageServeur: "Mauvaise connexion internet.",
            });
          } else {
  ...
  return (
    <>
      <PopupMessage
        message={
          socketStat.messageServeur !== false
            ? socketStat.messageServeur
            : false
        }
      />
      {socketStat.waitAuth &&
        !(props.pathname === "/login" || props.pathname === "/login/") && (
          <Loader>
            <Spinner
              color="purple"
              messageServeur={socketStat.messageServeur}
            />
          </Loader>
        )}
