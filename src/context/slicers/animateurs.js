...
export const { callApi, ApiCallBackData, ApiCallBackNoData, ApiCallBackFail } = AnimateursSlicer.actions;
			...
			await axios
				...
				.then(res => {
					res.data.statut === 'success'
						...
						: dispatch(ApiCallBackFail('La récupération des animateurs a échoué'));
				})
				.catch(error => {
					dispatch(
						ApiCallBackFail(
...
export const addAnimateurs = data => {
	return async dispatch => {
		...
		if (!!token) {
			await axios
				.post(`/api/animateurs?token=${token}`, data)
				.then(res => {
					res.data.statut === 'success'
		...
		if (!!token) {
			await axios
				...
				.then(res => {
					res.data.statut === 'success'
						? dispatch(ApiCallBackNoData())
						: dispatch(ApiCallBackFail("La suppression de l'animateur a échoué"));
				})
				.catch(error => {
					dispatch(
						ApiCallBackFail(
		...
		if (!!token) {
			await axios
				...
				.then(res => {
					res.data.statut === 'success'
						? dispatch(ApiCallBackNoData())
						: dispatch(ApiCallBackFail("La modification de l'animateur a échoué"));
				})
				.catch(error => {
					dispatch(
						ApiCallBackFail(
