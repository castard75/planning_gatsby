...
module.exports = db => {
	...
	router
		.post('/login', registerLimiter, fieldsMiddleware, async (req, res) => {
			...
			if (!user.statut) res.status(500).send('Une erreur est survenue avec votre identifiant.');
		...
		.post('/signup', [authMiddleware, fieldsMiddleware], async (req, res) => {
			...
			if (!user.statut) res.status(500).send('Une erreur est survenue avec vos identifiants.');
