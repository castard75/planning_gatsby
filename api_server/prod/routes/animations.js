...
module.exports = (app, db) => {
	...
	router
		.route('/animations')
		.get([authMiddleware, fieldsMiddleware], (req, res) => {
			const date = req.body?.date || req.query?.date || req.cookies?.date;
			dbController.get(
				'*',
				({ statut, error, results }) => sendData(statut, res, results, error),
				'animations',
				date
					? 'WHERE `date` >= "' + date + '-01" AND `date` <= "' + date + '-31"'
					: 'WHERE `date` >= DATE_SUB(NOW(), INTERVAL 4 MONTH)'
			);
		})
		.post([authMiddleware, fieldsMiddleware], (req, res) => {
			const numberOfAnimations = req.body?.number || req.query?.number || req.cookies?.number;
